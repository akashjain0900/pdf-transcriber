...
One-off migration, exposed as `ledger import-legacy`. It lives inside the
package rather than in tools/ so it is available in a bundled build too.

    ledger import-legacy transcriptions-backup-2026-08-28.json
    ledger import-legacy transcriptions-backup-2026-08-28.json --apply
...
from .db import PAGE_DONE, PAGE_FLAGGED, PAGE_PENDING


class ImportAborted(Exception):
    """The backup or the database is not in a state the import can work with."""
...
def run_import(config, database, backup: str, apply: bool = False) -> int:
    """
    Import an old HTML-app backup.

    Called by `ledger import-legacy`. Returns a process exit code.
    """
    backup_path = Path(backup)
    if not backup_path.exists():
        raise ImportAborted(f"No such file: {backup_path}")
